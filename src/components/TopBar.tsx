...
export function TopBar({ theme, onToggleTheme, onToggleLeftSidebar, onToggleRightSidebar, hideSidebarButtons = false }: TopBarProps) {
  ...
  const currentTime = new Date().toLocaleTimeString(t('locale') || 'ru-RU', { hour: '2-digit', minute: '2-digit' });
          ...
          {showStationInfo ? (
            <div className="hidden sm:block">
              <div className={`text-sm font-medium ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>
                {user?.station ? `${t('stationLabel') || 'Станция'} ${user.station}` : t('headerStation')}
              </div>
          ...
          {notifOpen && (
            ...
            >
              <div className={`p-3 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
                <h3 className={`font-semibold ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{t('notificationTitle')}</h3>
              </div>
        ...
        <div className="fixed inset-0 z-[70] bg-black/40 flex items-center justify-center p-4" onClick={() => setSelectedNotification(null)}>
          ...
          >
             <div className={`px-4 py-3 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
              <div className={`font-semibold ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>{t('notificationDetails')}</div>
            </div>
